...
class Spectra {
    ...
    setupEventListeners() {
        ...
        document.getElementById('penStrength').addEventListener('input', (e) => this.penStrength = parseFloat(e.target.value));
        document.getElementById('lineStartStrength').addEventListener('input', (e) => this.lineStartStrength = parseFloat(e.target.value));
        document.getElementById('lineEndStrength').addEventListener('input', (e) => this.lineEndStrength = parseFloat(e.target.value));
        document.getElementById('lineHarmonics').addEventListener('input', (e) => this.lineHarmonics = parseInt(e.target.value));
        document.getElementById('harmonicRolloff').addEventListener('input', (e) => this.harmonicRolloff = parseFloat(e.target.value));
        ...
        document.getElementById('alignLeftBtn').addEventListener('click', () => this.alignLeft());
        document.getElementById('alignRightBtn').addEventListener('click', () => this.alignRight());
    ...
    renderWaveform() {
        ...
        if (this.renderedWaveform && this.renderedWaveform.length > 0) {
            ...
            const startSampleView = Math.floor(this.viewStart * sampleRate);
            const endSampleView = Math.floor(this.viewEnd * sampleRate);
            const viewSamples = endSampleView - startSampleView;
            ...
            for (let x = 0; x < width; x++) {
                const sampleOffset = Math.floor(x * viewSamples / width);
                const startSample = startSampleView + sampleOffset;
                const endSample = Math.min(startSample + samplesPerPixel, this.renderedWaveform.length);

                let min = 0;
                let max = 0;

                for (let i = startSample; i < endSample && i >= 0 && i < this.renderedWaveform.length; i++) {
            ...
            for (let x = 0; x < width; x++) {
                ...
                for (let i = startSample; i < endSample && i >= 0 && i < channelData.length; i++) {
    ...
    handleMouseDown(e) {
        ...
        // Shift+drag or Alt+drag to pan the view
    ...
    alignLeft() {
        ...
        // Find the leftmost (earliest) start time among all selected partials
        ...
        // Shift each partial so its start aligns with the leftmost start
        for (const partial of selectedPartialsList) {
            if (partial.points.length > 0) {
                const shift = minStartTime - partial.points[0].time;
                for (const point of partial.points) {
                    point.time += shift;
                }
            }
        ...
        this.render();
        this.updateRenderedWaveform();
        this.notify(`Aligned ${selectedPartialsList.length} partials to left`);
    }
    ...
    alignRight() {
        ...
        // Find the rightmost (latest) end time among all selected partials
        ...
        // Shift each partial so its end aligns with the rightmost end
        for (const partial of selectedPartialsList) {
            if (partial.points.length > 0) {
                const shift = maxEndTime - partial.points[partial.points.length - 1].time;
                for (const point of partial.points) {
                    point.time += shift;
                }
            }
        ...
        this.render();
        this.updateRenderedWaveform();
        this.notify(`Aligned ${selectedPartialsList.length} partials to right`);
    }
